...
const Icon = ({ item, type }: { item: Trendable, type: TooltipType, }) => {
  ...
  switch (trend) {
    case `up`:
      return (
        <span className="pointer material-symbols-outlined green">
          trending_up
        </span>
      );
    case `down`:
      return (
        <span className="pointer material-symbols-outlined red">
          trending_down
        </span>
      );
    case `same`:
      return (
        <span className="pointer material-symbols-outlined">trending_flat</span>
      );
    default:
      return (
        <span className="pointer material-symbols-outlined">unknown_med</span>
      );
  }
};
...
const Tooltip = (props: TooltipProps) => {
  ...
  const trendDetails = (
    <div className="f6">
      {props.item.trends.map((trend, idx) => {
        return (
          <div className="flex justify-between" key={idx}>
            ...
            {props.type == TooltipType.Usage && (
              <div>{toolbox.Formatter.decimalThousands(trend.usage)}</div>
            )}
          </div>
        );
      })}
      ...
      {!hasTrends && (
        <div>This resource was not available in previous billing periods</div>
      )}
    </div>
  );
