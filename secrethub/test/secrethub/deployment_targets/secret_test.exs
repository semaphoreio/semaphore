defmodule Secrethub.DeploymentTargets.SecretTest do
  ...
  describe "changeset/2" do
    test "casts necessary fields", %{params: params} do
      ...
      assert secret.dt_id == params[:dt_id]
    ...
    test "checks required fields", %{params: params} do
      required_fields = ~w(name dt_id org_id created_by updated_by)a
    ...
    test "validates name format", %{params: params} do
      ...
      assert %Ecto.Changeset{valid?: false, errors: [name: {^error_message, _}]} =
               Secret.changeset(%Secret{}, %{params | name: "secret!!!"})
