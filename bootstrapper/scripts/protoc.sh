...
INTERNAL_API_OUT=/app/pkg/protos
MODULE_NAME=github.com/semaphoreio/semaphore/bootstrapper
MODULES=(${1//,/ })
...
generate_proto_definition() {
  ...
  if [[ "${MODULE}" == "include/internal_api/response_status" ]]; then
    mkdir -p $INTERNAL_API_OUT/$MODULE
    protoc --proto_path $INTERNAL_API_FOLDER/include/internal_api \
          --go-grpc_out=$INTERNAL_API_OUT/$MODULE \
          --go-grpc_opt=paths=source_relative \
          --go-grpc_opt=require_unimplemented_servers=false \
          --go_out=$INTERNAL_API_OUT/$MODULE \
...
_set_go_package() {
  ...
  GO_PACKAGE="option go_package = \"github.com/semaphoreio/semaphore/bootstrapper/pkg/protos/$MODULE\";"
