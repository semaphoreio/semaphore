...
RSpec.describe RepoHost::Github::Payload do
  describe "PR approval commands" do
    ...
    it "recognizes /sem-approve when embedded in regular text" do
      payload = described_class.new(
        {
          "issue" => { "number" => 45, "pull_request" => { "url" => "https://example.test/pr/45" } },
          "comment" => { "body" => "LGTM /sem-approve", "user" => { "login" => "octocat" } }
        }.to_json
      )
    ...
    it "recognizes /sem-approve with trailing text but no options" do
      payload = described_class.new(
        {
          "issue" => { "number" => 45, "pull_request" => { "url" => "https://example.test/pr/45" } },
          "comment" => { "body" => "/sem-approve thanks", "user" => { "login" => "octocat" } }
        }.to_json
      )
    ...
    it "ignores options when unknown flags are also present" do
      payload = described_class.new(
        {
          "issue" => { "number" => 45, "pull_request" => { "url" => "https://example.test/pr/45" } },
          "comment" => { "body" => "/sem-approve --include-secrets --unknown-flag", "user" => { "login" => "octocat" } }
        }.to_json
      )
    ...
    it "collects options from all approval-command lines" do
      payload = described_class.new(
        {
          "issue" => { "number" => 45, "pull_request" => { "url" => "https://example.test/pr/45" } },
          "comment" => { "body" => "/sem-approve\n/sem-approve --enable-cache", "user" => { "login" => "octocat" } }
        }.to_json
      )
