defmodule Zebra.Workers.JobDeletionPolicyMarker do
  ...
  def handle_message(message) do
    with {:ok, decoded} <- decode_message(message),
      ...
      unmarked = batch_unmark_jobs_for_deletion(org_id, cutoff_date, days, batch_size, 0)
  ...
  defp batch_unmark_jobs_for_deletion(org_id, cutoff_date, deletion_days, batch_size, acc) do
    case Job.unmark_jobs_for_deletion(org_id, cutoff_date, deletion_days, batch_size) do
      {0, _} ->
        acc

      {unmarked, _} ->
        batch_unmark_jobs_for_deletion(org_id, cutoff_date, deletion_days, batch_size, acc + unmarked)
    end
  end

  defp get_batch_size do
    config = Application.get_env(:zebra, __MODULE__, [])
    Keyword.get(config, :batch_size, @default_limit)
  end
