    ...
    if(api.state.url != ``) {
      ...
      fetch(api.state.url)
        ...
        .then(jsonPayload => {
          ...
          if(navigation.state.activeReportId === `` || !loadedReports.find((report: any) => report.id === navigation.state.activeReportId)) {
...
const Loader = ({ state, children, encodedEmail, reportState }: { state: LoadingStore.State, children?: ComponentChildren, encodedEmail: string, reportState: ReportStore.State, }) => {
  if(state.loading) {
    return (
  ...
  return (
    <div className="bg-washed-red br3 pa3 pa4-m ba b--black-075 mt4 flex-auto">
      <div className="flex justify-between items-center">
        ...
        <a className="btn btn-secondary ml1" href={encodedEmail}>Request help</a>
      </div>
