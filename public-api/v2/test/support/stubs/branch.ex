defmodule Support.Stubs.Branch do
  ...
  @spec create(atom | %{:id => any, optional(any) => any}, keyword) :: any
  def create(project, params \\ []) do
    ...
    api_model = struct(Branch, params_with_defaults)
  ...
  defmodule Grpc do
    ...
    def describe(req, _) do
      case DB.all(:branches) |> filter(req) do
        [] ->
          %InternalApi.Branch.DescribeResponse{
    ...
    def list(req, _) do
      ...
      %InternalApi.Branch.ListResponse{
        status: status(:OK),
        branches: branches,
        page_number: 1,
