defmodule Projecthub.Models.DeployKey do
  ...
  def remove_from_github(deploy_key, repo, token) do
    with {:ok, owner} <- PathSegment.validate(repo.owner),
      ...
      {:error, :invalid_github_path_segment} = error ->
        Logger.error("Refusing to remove deploy key #{deploy_key.id} on github: unsafe repository owner/name")
  ...
  defp post_to_github(deploy_key, project, repo, token) do
    with {:ok, owner} <- PathSegment.validate(repo.owner),
      ...
      {:error, :invalid_github_path_segment} = error ->
        Logger.error("Refusing to create deploy key #{deploy_key.id} on github: unsafe repository owner/name")
