defmodule Support.Stubs.DB.Errors.NotFound do
...
defmodule Support.Stubs.DB do
  ...
  def upsert(table, entry) do
    case find_by(table, :id, entry.id) do
      nil -> insert(table, entry)
      _ -> update(table, entry)
    end
  end
  ...
  def update(table, new_entry) do
    verify_insert!(table, new_entry)

    new_table =
      all(table)
      |> Enum.map(fn old_entry ->
        if old_entry.id == new_entry.id do
          new_entry
        else
          old_entry
        end
  ...
  defp not_found!(table, column, value) do
    ...
    raise Support.Stubs.DB.Errors.NotFound, msg
  end
