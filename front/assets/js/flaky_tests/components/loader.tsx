...
export const LoadingFailed = ({ text, retry }: { text: string, retry?: boolean }) => {
  ...
  if(retry) {
    return <div className="pv2 flex flex-column items-center justify-center">
      <div className="mt1 red">{text}</div>
      <button className="mt2 btn btn-secondary btn-tiny" onClick={() => dispatchRequest({ type: `FETCH` })}>Retry</button>
    </div>;
  } else {
    return <div className="pv2 flex items-center justify-center">
      <div className="mt1 red">{text}</div>
