module Semaphore::Gitlab::Credentials
  @app_id = nil
  @secret_id = nil
  ...
  class Local
    def self.app_id
      App.gitlab_app_id.presence
    end

    def self.secret_id
      App.gitlab_secret_id.presence
    end
  end
  ...
  class InstanceConfigClient
    def self.fetch
      ...
      req = InternalApi::InstanceConfig::ListConfigsRequest.new(:types => [InternalApi::InstanceConfig::ConfigType::CONFIG_TYPE_GITLAB_APP])
    ...
    def self.get_field(field_name)
      ...
      config = response.configs.find { |conf| conf.type == :CONFIG_TYPE_GITLAB_APP and conf.state == :STATE_CONFIGURED }
