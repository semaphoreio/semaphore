defmodule PublicAPI.Handlers.DeploymentTargets.Cordon do
  @moduledoc """
  Plug Deactivates a deployment target.
  """
  ...
  @operation_id "DeploymentTargets.Deactivate"
  ...
  plug(PublicAPI.Plugs.Metrics, tags: ["deactivate", "deployment_targets"])
  ...
  def open_api_operation(_) do
    %Operation{
      ...
      summary: "Deactivate a project deployment targets",
        ...
        Responses.with_errors(%{
          200 =>
            Operation.response(
              ...
              "application/json",
              Schemas.DeploymentTargets.CordonResponse
            )
        })
    }
  end

  def deactivate(conn, _opts) do
    conn
    |> get_resource_id()
    |> case do
      nil ->
        Logger.debug(
          "Error deactivating a deployment target: could not find the deployment target"
        )
    ...
    error ->
      conn
      |> handle_error(error, "deactivating")
  end
end
