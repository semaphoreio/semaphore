defmodule Support.Stubs.Deployments do
  ...
  defmodule Grpc do
    ...
    def mock_wrong_org(wrong_org_id) do
      GrpcMock.stub(DeploymentsMock, :describe, fn request, _opts ->
        %{target: target} =
          request |> Util.Proto.to_map!(transformations: transformations()) |> describe()

        target = %{target | organization_id: wrong_org_id}
        %API.DescribeResponse{target: target}
      end)
    end

    def mock_wrong_project(wrong_project_id) do
      GrpcMock.stub(DeploymentsMock, :describe, fn request, _opts ->
        %{target: target} =
          request |> Util.Proto.to_map!(transformations: transformations()) |> describe()

        target = %{target | project_id: wrong_project_id}
        %API.DescribeResponse{target: target}
      end)
    end
    ...
    defp list(%{project_id: project_id}) do
      ...
      %API.ListResponse{targets: targets}
    end
    ...
    defp describe(%{target_id: target_id, project_id: "", target_name: ""}) do
      case DB.find(:deployment_targets, target_id) do
        ...
        target -> %API.DescribeResponse{target: target.api_model}
      end
    end
    ...
    defp history(%{target_id: target_id}) do
      case DB.find(:deployment_targets, target_id) do
        nil -> raise GRPC.RPCError, status: :not_found, message: "Target not found"
        target -> %API.HistoryResponse{deployments: target.history}
      end
    end
    ...
    defp cordon(%{target_id: target_id, cordoned: cordoned}) do
      case DB.find(:deployment_targets, target_id) do
        ...
        target ->
          ...
          %API.CordonResponse{target_id: target_id, cordoned: cordoned}
      end
    end
    ...
    defp create(request = %{target: target_params, secret: secret_params}) do
      ...
      target_params = target_params |> Map.from_struct()
      secret_params = secret_params |> Map.from_struct()

      subject_rules =
        Enum.map(target_params.subject_rules, fn rule ->
          %{
            type: InternalApi.Gofer.DeploymentTargets.SubjectRule.Type.value(rule.type),
            subject_id: rule.subject_id
          }
        end)

      object_rules =
        Enum.map(target_params.object_rules, fn rule ->
          %{
            type: InternalApi.Gofer.DeploymentTargets.ObjectRule.Type.value(rule.type),
            match_mode:
              InternalApi.Gofer.DeploymentTargets.ObjectRule.Mode.value(rule.match_mode),
            pattern: rule.pattern
          }
        end)

      target_params = %{target_params | subject_rules: subject_rules, object_rules: object_rules}
      ...
      %API.CreateResponse{target: target.api_model}
    end
    ...
    defp update(request = %{target: target_params, secret: secret_params}) do
      ...
      target_params =
        %{target_params | subject_rules: subject_rules, object_rules: object_rules}
        |> Map.put(:organization_id, target.api_model.organization_id)
      ...
      %API.UpdateResponse{target: target.api_model}
    end
    ...
    defp delete(%{target_id: target_id, requester_id: _requester_id}) do
      DB.find(:deployment_targets, target_id)
      |> case do
        nil ->
          raise GRPC.RPCError, status: :not_found, message: "Deployment target not found"
        ...
        _target ->
          ...
          %API.DeleteResponse{target_id: target_id}
      end
    end
    ...
    defp target_api_model(target_id, request) do
      ...
      %API.DeploymentTarget{
        ...
        url: target.url,
        subject_rules: request.target.subject_rules,
        object_rules: request.target.object_rules
      }
    end
    ...
    defp validate_subject_rule(params) do
      {%API.SubjectRule{}, %{type: :integer, subject_id: :string}}
    ...
    defp validate_object_rule(params) do
      {%API.ObjectRule{}, %{type: :integer, match_mode: :integer, pattern: :string}}
