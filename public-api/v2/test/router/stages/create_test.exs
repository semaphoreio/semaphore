defmodule Router.Stages.CreateTest do
  use PublicAPI.Case
  ...
  describe "authorized users" do
    ...
    test "create a stage", ctx do
      ...
      {:ok, response} = create_stage(ctx, stage)
      created_stage = Jason.decode!(response.body)

      assert 200 == response.status_code
      check_response(created_stage)
      assert stage.metadata.name == created_stage["metadata"]["name"]
    end
  ...
  defp create_stage(ctx, stage) do
    url = url() <> "/canvases/#{ctx.canvas_id}/stages"
