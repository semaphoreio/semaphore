...
export default class HistoryPage {
  static init() {
    const params = window.InjectedDataByBackend.Tasks
  ...
  handleTextFilters() {
    this.handleTextFilterApplied('branch_name')
    this.handleTextFilterApplied('pipeline_file')
    ...
    handleSubmit: function (result) {
      if (result.display_name) {
        historyPage.applyFiltersAndRefreshPage({
          branch_name: result.display_name
        })
      }
    },
