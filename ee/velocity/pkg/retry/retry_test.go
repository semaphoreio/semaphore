...
func ExampleWithConstantWait() {
	maxAttempts := 10
	waitDuration := 10 * time.Second

	err := WithConstantWait("RabbitMQ connection", maxAttempts, waitDuration, func() error {
		// Do something that may fail and needs to retry.
		// e.g.: consumer.Start(&options)
		return nil
	})
	fmt.Println(err)
	// Output: <nil>
