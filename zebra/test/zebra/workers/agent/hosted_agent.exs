defmodule Zebra.Workers.Agent.HostedAgentTest do
  ...
  describe ".occupy" do
    ...
    test "uses original e2 machine values when e2_to_f1 migration flag is disabled" do
      {:ok, job} =
        Support.Factories.Job.create(:scheduled, %{
          machine_type: "e2-standard-2",
          machine_os_image: "ubuntu2004"
    ...
    test "remaps e2-standard-2 machines to f1-standard-2 when migration flag is enabled" do
      org_id = Support.StubbedProvider.e2_to_f1_org_id()
    ...
    test "remaps e2-standard-4 machines to f1-standard-4 when migration flag is enabled" do
      org_id = Support.StubbedProvider.e2_to_f1_org_id()
    ...
    test "does not remap e1 machines when only e2_to_f1 migration flag is enabled" do
      org_id = Support.StubbedProvider.e2_to_f1_org_id()
    ...
    test "translates both e1 and e2 machines when both flags are enabled" do
      e1_org_id = Support.StubbedProvider.e1_to_f1_org_id()
      e2_org_id = Support.StubbedProvider.e2_to_f1_org_id()

      {:ok, e1_job} =
        Support.Factories.Job.create(:scheduled, %{
          organization_id: e1_org_id,
          machine_type: "e1-standard-2",
          machine_os_image: "ubuntu2004"
        })
      ...
      with_mock Stub,
        ...
        assert Agent.occupy(e1_job) == {:ok, Agent.construct_agent(response)}
        assert Agent.occupy(e2_job) == {:ok, Agent.construct_agent(response)}
      end
    end
  end
