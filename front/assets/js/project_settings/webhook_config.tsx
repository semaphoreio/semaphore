...
const App = (props: { config: Config }) => {
  ...
  const regenerate = () => {
    ...
    if (confirmed) {
      const url = new toolbox.APIRequest.Url<{ secret: string, message: string, endpoint: string }>(
        `post`,
        webhook.regenerateUrl
      );
      void url
        .call()
        .then((resp) => {
      ...
      <div className="mb1 flex justify-between items-center">
        ...
        <div>
          <toolbox.Popover
            content={
              <div className="f6">
                Webhooks allow your server to receive real-time updates. Ensure the webhook URL is correctly configured
                to handle incoming events and validate their signatures for security.
                <br/>
