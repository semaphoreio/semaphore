defmodule Guard.McpOAuth.Server do
  ...
  alias Guard.Store.{McpOAuthAuthCode, McpOAuthClient, RbacUser}
      ...
      {:ok, client} ->
        if McpOAuthClient.valid_redirect_uri?(client, redirect_uri) do
          ...
          case Guard.McpGrant.Actions.create(grant_params) do
            {:ok, grant} ->
              Logger.info("[McpOAuth.Server] Created grant #{grant.id} for user=#{user_id}")
            ...
            {:error, reason} ->
              Logger.error("[McpOAuth.Server] Failed to create grant: #{inspect(reason)}")
              error_url = Authorize.build_error_redirect(redirect_uri, "server_error", "Failed to create grant", state)
  ...
  defp parse_tool_scopes(scope_string) when is_binary(scope_string) do
    # Extract tool scopes from the scope string
    # The scope string may contain "mcp" plus tool-specific scopes
    scope_string
    |> String.split(" ")
    |> Enum.filter(&(String.trim(&1) != "" and &1 != "mcp"))
