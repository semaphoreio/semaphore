defmodule Guard.FrontRepo.RepoHostAccount do
  ...
  # Shape-only error summary for log output. Never inspect the raw changeset —
  # its `changes` map carries OAuth access/refresh tokens that would otherwise
  # leak into any log sink.
  defp changeset_error_fields(%Ecto.Changeset{errors: errors}) do
    Enum.map_join(errors, ",", fn {field, {msg, _opts}} -> "#{field}:#{msg}" end)
  end

  defp changeset_error_fields(atom) when is_atom(atom), do: "non_changeset:#{atom}"
  defp changeset_error_fields(_other), do: "non_changeset:opaque"
end
