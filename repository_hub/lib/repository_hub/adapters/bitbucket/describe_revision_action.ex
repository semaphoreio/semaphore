defimpl RepositoryHub.Server.DescribeRevisionAction, for: RepositoryHub.BitbucketAdapter do
  ...
  def execute(adapter, request) do
    with {:ok, context} <- BitbucketAdapter.context(adapter, request.repository_id),
         {:ok, reference} <- get_reference(request.revision.reference, context.repository, context.bitbucket_token),
  ...
  defp get_reference(reference, repository, bitbucket_token) do
    BitbucketClient.get_reference(
      %{
