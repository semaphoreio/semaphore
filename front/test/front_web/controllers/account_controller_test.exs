defmodule FrontWeb.AccountControllerTest do
  ...
  describe "POST refresh_github" do
    setup do
      Support.Stubs.Feature.setup_feature("ui_show_refresh_github_profile",
        state: :ENABLED,
        quantity: 1
      )
    ...
    test "returns 404 when feature is disabled", %{conn: conn} do
      ...
      conn = post(conn, "/account/refresh_github")

      assert response(conn, 404)
    end

    test "flashes notice and redirects when login is up to date", %{conn: conn} do
      ...
      assert redirected_to(conn, 302) == "/account"
      assert get_flash(conn, :notice) =~ "already up to date"
    end

    test "flashes new login and redirects when GitHub login changed", %{conn: conn} do
      GrpcMock.stub(UserMock, :refresh_github_profile, fn _, _ ->
        InternalApi.User.RefreshGithubProfileResponse.new(
          status: InternalApi.User.RefreshGithubProfileResponse.Status.value(:UPDATED),
          login: "radwo-renamed"
        )
      end)
      ...
      assert redirected_to(conn, 302) == "/account"
      assert get_flash(conn, :notice) =~ "GitHub username updated to radwo-renamed"
    end

    test "flashes alert when the gRPC call fails", %{conn: conn} do
      GrpcMock.stub(UserMock, :refresh_github_profile, fn _, _ ->
      ...
      assert redirected_to(conn, 302) == "/account"
      assert get_flash(conn, :alert) =~ "Could not refresh GitHub data"
    end

    test "rejects the second call within the cooldown window", %{conn: conn} do
      GrpcMock.stub(UserMock, :refresh_github_profile, fn _, _ ->
        InternalApi.User.RefreshGithubProfileResponse.new(
          status: InternalApi.User.RefreshGithubProfileResponse.Status.value(:NO_CHANGE),
          login: "radwo"
      ...
      _conn = post(conn, "/account/refresh_github")

      second_conn = post(conn, "/account/refresh_github")

      assert redirected_to(second_conn, 302) == "/account"
      assert get_flash(second_conn, :alert) =~ "Please wait a moment"
    end
  end
end
