defmodule Rbac.GrpcServers.GroupsServer do
  ...
  def modify_group(%Groups.ModifyGroupRequest{} = req, _stream) do
    ...
    case Group.fetch_group(req.group.id) do
      {:ok, group} ->
        authorize!(@manage_groups_permission, req.requester_id, group.org_id)
        check_if_members_are_in_org!(req.members_to_add, group.org_id)

        case Group.modify_metadata(group.id, group.org_id, req.group.name, req.group.description) do
          {:ok, modified_group} ->
            {:ok, _} =
              create_request(
