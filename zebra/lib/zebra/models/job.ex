defmodule Zebra.Models.Job do
  ...
  defp publish_single_job_deletion(channel, exchange_name, routing_key, job_meta) do
    # Extract values from tuple {job_id, org_id, project_id, artifact_store_id}
    {id, organization_id, project_id, artifact_store_id} = job_meta

    {ok, organization_id_str} = Ecto.UUID.load(organization_id)
    {ok, project_id_str} = Ecto.UUID.load(project_id)
