...
func TestArtifactsListSuccessJobScope(t *testing.T) {
	...
	if artifactClient.lastList.GetLimit() != 200 {
		t.Fatalf("expected list limit 200, got %d", artifactClient.lastList.GetLimit())
	}
...
func TestArtifactsSignedURLSuccessWorkflowScope(t *testing.T) {
	...
	if artifactClient.lastSigned == nil {
		t.Fatalf("expected GetSignedURLS request to be recorded")
	}
	...
	if artifactClient.lastSigned.GetLimit() != int32(maxListItems) {
		t.Fatalf("expected signed URL limit %d, got %d", maxListItems, artifactClient.lastSigned.GetLimit())
	}
	if artifactClient.lastSigned.GetArtifactId() != testArtifactStore {
		t.Fatalf("expected artifact_id %s, got %s", testArtifactStore, artifactClient.lastSigned.GetArtifactId())
	}
	if workflowClient.LastDescribe == nil || workflowClient.LastDescribe.GetWfId() != testWorkflowID {
		t.Fatalf("expected workflow describe request for %s", testWorkflowID)
	}
...
func TestArtifactsSignedURLReturnsMultipleURLsForDirectoryPath(t *testing.T) {
	artifactClient := &artifacthubClientStub{
		signedURLs: []*artifacthubpb.SignedURL{
			{Url: "https://example.com/a.log", Method: artifacthubpb.SignedURL_HEAD},
			{Url: "https://example.com/b.log", Method: artifacthubpb.SignedURL_HEAD},
		},
	}
	provider := &support.MockProvider{
		Timeout:           time.Second,
	...
	if len(result.URLs) != 2 {
		t.Fatalf("expected 2 signed URLs, got %d", len(result.URLs))
	}
	if result.URLs[0].URL != "https://example.com/a.log" {
		t.Fatalf("unexpected first signed URL, got %s", result.URLs[0].URL)
	}
	if result.URLs[0].Method != "HEAD" || result.URLs[1].Method != "HEAD" {
		t.Fatalf("expected HEAD methods, got %#v", result.URLs)
	}
	if artifactClient.lastSigned == nil {
		t.Fatalf("expected GetSignedURLS request to be recorded")
	}
	expectedPath := fmt.Sprintf("artifacts/projects/%s/reports", testProjectID)
	if artifactClient.lastSigned.GetPath() != expectedPath {
		t.Fatalf("expected signed path %s, got %s", expectedPath, artifactClient.lastSigned.GetPath())
	}
}
...
func TestArtifactsListLimitMetadata(t *testing.T) {
	artifactClient := &artifacthubClientStub{
		listResp: &artifacthubpb.ListPathResponse{
			Items: []*artifacthubpb.ListItem{
	...
	if artifactClient.lastList.GetLimit() != 2 {
		t.Fatalf("expected list limit 2, got %d", artifactClient.lastList.GetLimit())
	}
}
...
func TestArtifactsListRootEmptyReturnsSuccess(t *testing.T) {
	...
	if result.Page.Limit != defaultListLimit || result.Page.Returned != 0 {
...
func TestArtifactsListArtifacthubTooManyFilesError(t *testing.T) {
	provider := &support.MockProvider{
		Timeout: time.Second,
		ArtifacthubClient: &artifacthubClientStub{
			listErr: grpcstatus.Error(grpccodes.FailedPrecondition, "path resolves to too many files; narrow the path"),
		},
		JobClient: &jobClientStub{orgID: testOrgID, projectID: testProjectID},
		ProjectClient: &support.ProjectClientStub{
			Response: newProjectDescribeResponse(testOrgID, testProjectID, testArtifactStore),
	...
	msg := requireErrorText(t, res)
	if !strings.Contains(msg, fmt.Sprintf("maximum %d per request", defaultListLimit)) {
		t.Fatalf("expected too-many-files guidance, got %q", msg)
	}
}
...
func TestArtifactsSignedURLArtifacthubRPCError(t *testing.T) {
	...
	msg := requireErrorText(t, res)
	if !strings.Contains(msg, "artifacthub GetSignedURLS failed") {
		t.Fatalf("expected artifacthub signed URL failure, got %q", msg)
	}
}

func TestArtifactsSignedURLFallsBackToGetSignedURLWhenGetSignedURLSUnimplemented(t *testing.T) {
	artifactClient := &artifacthubClientStub{
		listResp: &artifacthubpb.ListPathResponse{
			Items: []*artifacthubpb.ListItem{
				{Name: fmt.Sprintf("artifacts/jobs/%s/agent/job_logs.txt.gz", testJobID), IsDirectory: false, Size: 123},
		...
		signedErr:       grpcstatus.Error(grpccodes.Unimplemented, "unknown method GetSignedURLS"),
		singleSignedURL: "https://example.com/fallback-signed",
	}
	provider := &support.MockProvider{
		Timeout:           time.Second,
		ArtifacthubClient: artifactClient,
	...
	if result.URLs[0].URL != "https://example.com/fallback-signed" {
		t.Fatalf("unexpected signed URL: %s", result.URLs[0].URL)
	}
	if result.URLs[0].Method != "HEAD" {
		t.Fatalf("expected signed URL method HEAD, got %s", result.URLs[0].Method)
	...
	if artifactClient.lastSingleSigned == nil {
		t.Fatalf("expected GetSignedURL fallback request to be recorded")
	}
	...
	if artifactClient.lastSingleSigned.GetPath() != expectedPath {
		t.Fatalf("expected fallback signed path %s, got %s", expectedPath, artifactClient.lastSingleSigned.GetPath())
	}
	if artifactClient.lastSingleSigned.GetMethod() != "HEAD" {
		t.Fatalf("expected fallback signed method HEAD, got %s", artifactClient.lastSingleSigned.GetMethod())
	}
}

func TestArtifactsSignedURLUnimplementedFallbackValidatesViaParentDirectoryListing(t *testing.T) {
	filePath := fmt.Sprintf("artifacts/projects/%s/reports/build.log", testProjectID)
	parentPath := fmt.Sprintf("artifacts/projects/%s/reports/", testProjectID)

	artifactClient := &artifacthubClientStub{
		listRespByPath: map[string]*artifacthubpb.ListPathResponse{
			parentPath: {
				Items: []*artifacthubpb.ListItem{
					{Name: filePath, IsDirectory: false, Size: 100},
				},
			},
		},
		signedErr:       grpcstatus.Error(grpccodes.Unimplemented, "unknown method GetSignedURLS"),
		singleSignedURL: "https://example.com/fallback-parent-validated",
	}
	provider := &support.MockProvider{
		Timeout:           time.Second,
		ArtifacthubClient: artifactClient,
	...
	if artifactClient.lastList == nil {
		t.Fatalf("expected fallback validation list request to be recorded")
	}
	if artifactClient.lastList.GetPath() != parentPath {
		t.Fatalf("expected parent directory list path %s, got %s", parentPath, artifactClient.lastList.GetPath())
	}
	if artifactClient.lastSingleSigned == nil {
		t.Fatalf("expected GetSignedURL fallback request to be recorded")
	}
	if artifactClient.lastSingleSigned.GetPath() != filePath {
		t.Fatalf("expected fallback signed path %s, got %s", filePath, artifactClient.lastSingleSigned.GetPath())
	}
}

func TestArtifactsSignedURLUnimplementedDirectoryFallbackReturnsUpgradeError(t *testing.T) {
	artifactClient := &artifacthubClientStub{
		listResp: &artifacthubpb.ListPathResponse{
			Items: []*artifacthubpb.ListItem{
				{Name: fmt.Sprintf("artifacts/projects/%s/reports/a.log", testProjectID), IsDirectory: false, Size: 10},
				{Name: fmt.Sprintf("artifacts/projects/%s/reports/b.log", testProjectID), IsDirectory: false, Size: 20},
			},
		},
		signedErr: grpcstatus.Error(grpccodes.Unimplemented, "unknown method GetSignedURLS"),
	}
	...
	msg := requireErrorText(t, res)
	if !strings.Contains(msg, "directory signed URLs require artifacthub GetSignedURLS RPC") {
		t.Fatalf("expected upgrade-required error, got %q", msg)
	}
	if artifactClient.lastSingleSigned != nil {
		t.Fatalf("expected no GetSignedURL fallback call for directory path")
	}
}

func TestArtifactsSignedURLUnimplementedFallbackTooManyParentEntriesReturnsUpgradeError(t *testing.T) {
	parentPath := fmt.Sprintf("artifacts/projects/%s/reports/", testProjectID)
	artifactClient := &artifacthubClientStub{
		listErrByPath: map[string]error{
			parentPath: grpcstatus.Error(grpccodes.FailedPrecondition, "path resolves to too many files"),
		},
		signedErr: grpcstatus.Error(grpccodes.Unimplemented, "unknown method GetSignedURLS"),
	}
	provider := &support.MockProvider{
	...
	if artifactClient.lastSingleSigned != nil {
		t.Fatalf("expected no GetSignedURL fallback call when parent listing is capped")
	}
}

func TestArtifactsSignedURLUnimplementedFallbackMissingFileReturnsNotFound(t *testing.T) {
	artifactClient := &artifacthubClientStub{
		listResp:   &artifacthubpb.ListPathResponse{Items: []*artifacthubpb.ListItem{}},
		signedErr:  grpcstatus.Error(grpccodes.Unimplemented, "unknown method GetSignedURLS"),
	...
	msg := requireErrorText(t, res)
	if msg != "artifact path not found" {
		t.Fatalf("expected path not found message, got %q", msg)
	}
	if artifactClient.lastSingleSigned != nil {
		t.Fatalf("expected no GetSignedURL fallback call for missing file")
	}
}
...
func TestArtifactsSignedURLArtifacthubPathNotFoundErrorMapped(t *testing.T) {
	provider := &support.MockProvider{
		Timeout: time.Second,
		ArtifacthubClient: &artifacthubClientStub{
			signedErr: grpcstatus.Error(grpccodes.NotFound, "whatever"),
		},
		JobClient: &jobClientStub{orgID: testOrgID, projectID: testProjectID},
		ProjectClient: &support.ProjectClientStub{
			Response: newProjectDescribeResponse(testOrgID, testProjectID, testArtifactStore),
...
func TestArtifactsSignedURLArtifacthubStoreNotFoundErrorMapped(t *testing.T) {
	provider := &support.MockProvider{
		Timeout: time.Second,
		ArtifacthubClient: &artifacthubClientStub{
			signedErr: grpcstatus.Error(grpccodes.NotFound, "artifact store not found"),
		},
		JobClient: &jobClientStub{orgID: testOrgID, projectID: testProjectID},
		ProjectClient: &support.ProjectClientStub{
			Response: newProjectDescribeResponse(testOrgID, testProjectID, testArtifactStore),
...
func TestArtifactsSignedURLDropsUnexpectedMethodsFromGetSignedURLS(t *testing.T) {
	artifactClient := &artifacthubClientStub{
		signedURLs: []*artifacthubpb.SignedURL{
			{Url: "https://example.com/delete", Method: artifacthubpb.SignedURL_DELETE},
			{Url: "https://example.com/head", Method: artifacthubpb.SignedURL_HEAD},
		},
	}
	provider := &support.MockProvider{
		Timeout:           time.Second,
	...
	if len(result.URLs) != 1 {
		t.Fatalf("expected 1 signed URL after dropping unexpected methods, got %d", len(result.URLs))
	}
	if result.URLs[0].URL != "https://example.com/head" {
		t.Fatalf("expected HEAD URL to remain, got %s", result.URLs[0].URL)
	}
	if result.URLs[0].Method != "HEAD" {
		t.Fatalf("expected HEAD method to remain, got %s", result.URLs[0].Method)
	}
}

func TestArtifactsSignedURLArtifacthubTooManyFilesError(t *testing.T) {
	provider := &support.MockProvider{
		Timeout: time.Second,
		ArtifacthubClient: &artifacthubClientStub{
			signedErr: grpcstatus.Error(grpccodes.FailedPrecondition, "path resolves to too many files; narrow the path"),
		},
		JobClient: &jobClientStub{orgID: testOrgID, projectID: testProjectID},
		ProjectClient: &support.ProjectClientStub{
			Response: newProjectDescribeResponse(testOrgID, testProjectID, testArtifactStore),
	...
	msg := requireErrorText(t, res)
	if !strings.Contains(msg, "maximum 1000 per request") {
		t.Fatalf("expected too-many-files guidance, got %q", msg)
	}
}
...
type artifacthubClientStub struct {
	...
	lastSigned       *artifacthubpb.GetSignedURLSRequest
...
func (s *artifacthubClientStub) GetSignedURLS(ctx context.Context, in *artifacthubpb.GetSignedURLSRequest, opts ...grpc.CallOption) (*artifacthubpb.GetSignedURLSResponse, error) {
	s.lastSigned = in
	if s.signedErr != nil {
		return nil, s.signedErr
	}
	if len(s.signedURLs) > 0 {
		return &artifacthubpb.GetSignedURLSResponse{Urls: s.signedURLs}, nil
	}
	if s.signedURL != "" {
		method := artifacthubpb.SignedURL_GET
		if strings.EqualFold(in.GetMethod(), "HEAD") {
			method = artifacthubpb.SignedURL_HEAD
		}
		return &artifacthubpb.GetSignedURLSResponse{
			Urls: []*artifacthubpb.SignedURL{
				{
					Url:    s.signedURL,
					Method: method,
				},
	...
	return &artifacthubpb.GetSignedURLSResponse{}, nil
}
