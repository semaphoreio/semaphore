defmodule Ppl.TaskClient.AfterPipelineTest do
  ...
  describe "start/4 execution_time_limit" do
    test "a job without its own limit inherits the pipeline's limit" do
      [job] = schedule_jobs([job_def("Submit Reports")], 45)

      assert job["execution_time_limit"] == 45
    end

    test "a job with its own limit keeps it" do
      raw_job =
        "Process Reports"
        |> job_def()
        |> Map.put("execution_time_limit", %{"minutes" => 20})

      [job] = schedule_jobs([raw_job], 45)

      assert job["execution_time_limit"] == 20
    end
    ...
    test "the pipeline's limit is inherited by every job that lacks one" do
      ...
      assert [first, second] = schedule_jobs(raw_jobs, 30)
      assert first["execution_time_limit"] == 30
      assert second["execution_time_limit"] == 60
    end

    test "no limit is set when the pipeline has no limit of its own" do
      [job] = schedule_jobs([job_def("Submit Reports")], nil)
  ...
  defp schedule_jobs(raw_jobs, ppl_limit) do
    test_pid = self()
    ...
    with_mock TaskApiClient,
      schedule: fn _definition, request_params, _url ->
        send(test_pid, {:scheduled, request_params})
        {:ok, %{id: UUID.uuid4()}}
      end do
      assert {:ok, _task_id} = AfterPipeline.start(ppl_req, after_ppl, ppl_trace(), ppl)
    end

    assert_received {:scheduled, request_params}
