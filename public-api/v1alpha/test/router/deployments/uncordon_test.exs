defmodule Router.Deployments.UncordonTest do
  use ExUnit.Case
  ...
  describe "PATCH /deployment_targets/:id/activate - uncordons the deployment" do
    ...
    test "when project ID doesn't match then returns error", ctx do
      ...
      {status_code, _headers, _body} =
        patch_uncordon(target.id, ctx, %{"project_id" => ctx.extra_args["project_id"]})
    ...
    test "when params are valid and state is off then uncordons a target",
      ...
      {status_code, _headers, _body} = patch_uncordon(target_id, ctx, params)
    ...
    test "when params are valid and project_id is missing state is off then uncordons a target",
         ctx = %{target_id: target_id} do
      target = Support.Stubs.DB.find(:deployment_targets, target_id)
      target = %{target | api_model: %{target.api_model | cordoned: true}}
      Support.Stubs.DB.upsert(:deployment_targets, target)
    ...
    test "when target doesn't exist then returns error", ctx do
      ...
      {status_code, _headers, message} = patch_uncordon(target_id, ctx, params)
      assert status_code == 404
    ...
    test "when params are valid but organization doesn't have feature enabled then returns error",
      ...
      {status_code, _headers, message} = patch_uncordon(ctx.target_id, ctx, params)
    ...
    test "when target id is invalid doesn't exist then returns error", ctx do
      target_id = "00000000-0000-3000-8000-00000000g000"
  ...
  defp patch_uncordon(target_id, ctx, params) do
    res = patch_uncordon_request(target_id, params, ctx)
    {:ok, response} = res
  ...
  defp patch_uncordon_request(target_id, params, ctx) do
    url =
      url() <>
        "/deployment_targets/" <> target_id <> "/activate?" <> Plug.Conn.Query.encode(params)
