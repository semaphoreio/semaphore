...
import (
	...
	pb "github.com/semaphoreio/semaphore/self_hosted_hub/pkg/protos/feature"
)
...
func (p *FeatureHubProvider) ListFeaturesWithContext(ctx context.Context, orgID string) ([]OrganizationFeature, error) {
	...
	conn, err := grpc.NewClient(p.grpcEndpoint, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		return nil, err
	}
