...
func TestDescribeTask_SurfacesParameterRegexFields(t *testing.T) {
	...
	if len(result.Task.Parameters) != 2 {
		t.Fatalf("expected 2 parameters, got %d", len(result.Task.Parameters))
	}
	...
	if !strings.Contains(textOut, "pattern (enforced): `^[a-zA-Z.]+\\.yml`") {
		t.Errorf("expected enforced regex line in markdown, got:\n%s", textOut)
	}
}
