defmodule Guard.McpOAuth.Token do
  ...
  defp create_token(auth_code) do
    # Fetch the grant to get tool_scopes
    case Guard.Store.McpGrant.find(auth_code.grant_id) do
      {:ok, grant} ->
        JWT.create_token(%{
          user_id: auth_code.user_id,
          grant_id: auth_code.grant_id,
          tool_scopes: grant.tool_scopes
        })

      {:error, :not_found} ->
        Logger.error("[McpOAuth.Token] Grant #{auth_code.grant_id} not found")
        {:error, error_response("server_error", "Associated grant not found")}
    end
  end
