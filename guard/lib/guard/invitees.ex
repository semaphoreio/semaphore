defmodule Guard.Invitees do
  ...
  defp http_call(resource, token, provider) when provider == "github" and is_binary(token) and token != "" do
  ...
  defp extract_uid(login, %{status_code: 404}, _),
    do: {:error, "Login #{login} cannot be found. Please check the login and try again or contact support if the problem persists."}

  defp extract_uid(login, %{status_code: status_code}, _),
    do: {:error, "error finding #{login}: #{status_code}. Try again later or contact support if the problem persists."}
