defmodule RepositoryHub.Server.GitLab.ClearExternalDataActionTest do
  ...
  describe "GitLab ClearExternalDataAction" do
    ...
    test "should propagate error when remove_deploy_key fails", %{gitlab_adapter: adapter} do
      with_mock GitlabClient, [:passthrough], remove_deploy_key: fn _, _ -> {:error, :not_found} end do
        repository =
          RepositoryModelFactory.gitlab_repo(
            name: "repository",
            owner: "dummy"
    ...
    test "should propagate error when remove_webhook fails", %{gitlab_adapter: adapter} do
      with_mock GitlabClient, [:passthrough], remove_webhook: fn _, _ -> {:error, :not_found} end do
        repository =
          RepositoryModelFactory.gitlab_repo(
            name: "repository",
            owner: "dummy"
