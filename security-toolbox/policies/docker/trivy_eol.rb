...
class Policy::TrivyEOL < Policy
  ...
  def test
    ...
    if File.exist?("out/results.json")
      ...
      # Add EOL information to the report
      report["EOLStatus"] = {
        "detected" => eol_detected,
        "os" => {
          "family" => os_family,
          "name" => os_name
        }
      ...
      if eol_detected
        report["EOLStatus"]["severity"] = "HIGH"
        report["EOLStatus"]["message"] = "Operating system #{os_family} #{os_name} is End of Life (EOL)"
        report["EOLStatus"]["recommendation"] = "Upgrade to a supported OS version"
