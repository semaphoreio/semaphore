...
const TicketAction = ({ item }: { item: FlakyTestItem }) => {
  ...
  return (
    <toolbox.Popover
      ...
      anchor={
        <span
          className={`${className} ${hasTicketUrl ? `dark-brown` : `gray hover-black`}`}
  ...
  return (
    <div className="ma2">
      <div className="bg-white br3 tl pa3">
        ...
        <div id={markdownContentId} className="code-block bg-washed-gray ma2" style={{ overflow: auto }}></div>
        <small className="f6 db black-60 mb2">
          Use provided markdown data to create a ticket in the tool of your choice.
        </small>
...
const ResolveAction = ({
  ...
  resolved: boolean;
  setResolved: Dispatch<StateUpdater<boolean>>;
}) => {
  const onClick = () => {
    if (resolved) {
