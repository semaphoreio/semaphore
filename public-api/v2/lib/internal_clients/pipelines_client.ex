defmodule InternalClients.Pipelines do
  @moduledoc """
  ...
  alias InternalClients.Pipelines.{RequestFormatter, GrpcClient, ResponseFormatter}

  def describe(params) do
    Metrics.benchmark("PublicAPI.ppl_client", ["describe"], fn ->
      params
  ...
  def terminate(pipeline_id) do
    Metrics.benchmark("PublicAPI.ppl_client", ["terminate"], fn ->
      pipeline_id
  ...
  def list(query_params) do
    Metrics.benchmark("PublicAPI.ppl_client", ["list"], fn ->
      query_params
      |> RequestFormatter.form_list_request()
      |> GrpcClient.list()
      |> ResponseFormatter.process_list_response(query_params)
    end)
  end

  def describe_topology(pipeline_id) do
    Metrics.benchmark("PublicAPI.ppl_client", ["describe_topology"], fn ->
      pipeline_id
  ...
  def partial_rebuild(pipeline_id, request_token, user_id) do
    Metrics.benchmark("PublicAPI.ppl_client", ["partial_rebuild"], fn ->
  ...
  def validate_yaml(yaml) do
    Metrics.benchmark("PublicAPI.ppl_client", ["validate_yaml"], fn ->
      yaml
