defmodule Support.Stubs.Artifacthub do
  ...
  defp build_artifacts_from_path(path) do
    ...
    |> Enum.map(fn
      {path, 0} ->
        %InternalApi.Artifacthub.ListItem{name: path, is_directory: false}

      {path, _} ->
        %InternalApi.Artifacthub.ListItem{name: path, is_directory: true}
    end)
  end
  ...
  defmodule Grpc do
    ...
    def describe(req, _) do
      case DB.find_by(:artifacts_retention_policies, :id, req.artifact_id) do
        nil ->
          retention_policy = %InternalApi.Artifacthub.RetentionPolicy{
            project_level_retention_policies: [],
            workflow_level_retention_policies: [],
            job_level_retention_policies: []
          }

          %InternalApi.Artifacthub.DescribeResponse{retention_policy: retention_policy}

        entry ->
          %InternalApi.Artifacthub.DescribeResponse{retention_policy: entry.api_model}
      end
    end
    ...
    def list_path(req, _) do
      ...
      %Api.ListPathResponse{items: items}
    end
    ...
    def get_signed_url(req, _) do
      ...
      api_model = %InternalApi.Artifacthub.ListItem{
        name: Path.join(file_path),
        is_directory: false
      }
      ...
      %Api.GetSignedURLResponse{url: url}
    end
    ...
    def update_retention_policy(req, _) do
      ...
      %InternalApi.Artifacthub.UpdateRetentionPolicyResponse{retention_policy: policy}
    end
