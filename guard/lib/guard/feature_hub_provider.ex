defmodule Guard.FeatureHubProvider do
  ...
  def provide_default_machine(org_id, _opts \\ []) do
    grpc_call(
      fn channel ->
        req = InternalApi.Feature.ListOrganizationMachinesRequest.new(org_id: org_id)
        Stub.list_organization_machines(channel, req, timeout: 3_000)
    ...
    |> case do
      {:ok, response} ->
        ...
        case Enum.find(machines, &(&1.type == response.default_type)) do
          nil -> {:error, :not_found}
          machine -> {:ok, machine}
        end

      error ->
        log_error("FeatureHubProvider.provide_default_machine", error: error, org_id: org_id)
  ...
  defp connect(metric) do
    Application.fetch_env(:guard, :feature_api_endpoint)
    |> case do
      {:ok, endpoint} ->
        {:ok, endpoint}
  ...
  defp machine_from_grpc(%Machine{availability: availability} = machine) do
    %FeatureProvider.Machine{
      type: machine.type,
      platform: parse_platform(machine.platform),
      vcpu: machine.vcpu,
