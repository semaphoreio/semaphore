defmodule Zebra.Workers.JobDeletionPolicyWorker do
  ...
  def tick(worker) do
    ...
    Enum.each(orgs, fn org_id ->
      ...
      delete_stop_requests_until_empty(org_id, worker.days, worker.limit)
      delete_jobs_until_empty(org_id, worker.days, worker.limit)
    end)

    Logger.info("Cleanup tick completed.")
  end

  defp delete_stop_requests_until_empty(org_id, days, limit) do
  ...
  defp delete_jobs_until_empty(org_id, days, limit) do
    Stream.iterate(0, &(&1 + 1))
    |> Enum.reduce_while(0, fn _, _ ->
