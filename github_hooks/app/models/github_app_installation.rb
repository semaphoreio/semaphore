class GithubAppInstallation < ActiveRecord::Base
  ...
  def self.find_by_repository_slug!(repository_slug)
    joins(:installation_repositories).where(:github_app_installation_repositories => { :slug => normalize_slug(repository_slug) }).first!
  end
  ...
  def replace_repositories!(repositories)
    normalized_repositories = self.class.normalize_repositories(repositories)
    repositories_by_slug = normalized_repositories.index_by { |repository| repository["slug"] }
  ...
  def add_repositories!(repositories)
    replace_repositories!(self.repositories + repositories)
  end

  def remove_repositories_by_slug!(slugs)
    normalized_slugs = Array(slugs).filter_map { |slug| self.class.normalize_slug(slug) }
    return if normalized_slugs.empty?

    remaining_repositories = repositories.reject { |repository| normalized_slugs.include?(repository["slug"]) }
    replace_repositories!(remaining_repositories)
  end

  def update_repository_ids!(repositories)
    updates_by_slug = self.class.normalize_repositories(repositories).index_by { |repository| repository["slug"] }
    updated_repositories = self.repositories.map do |repository|
      update = updates_by_slug[repository["slug"]]
