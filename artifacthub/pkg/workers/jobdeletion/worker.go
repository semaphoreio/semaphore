...
func NewWorker(amqpURL string, client storage.Client) (*Worker, error) {
	...
	// We got a connection reset by peer while using tackle
	// We will then try connecting using the rabbit amqp091 library directly
	// that way we know tackle is not the issue
	log.Printf("Manual connection to rabbitmq")
	conn, err := amqp091.Dial(amqpURL)
	if err != nil {
		log.Printf("JobDeletion Worker: Failed to connect to AMQP URL %v", err)
		return nil, err
	}
	log.Printf("Connected to rabbitmq manually")

	log.Printf("Closing manual connection to rabbitmq")
	err = conn.Close()
	if err != nil {
		log.Printf("JobDeletion Worker: Failed to close AMQP connection %v", err)
		return nil, err
	}
	log.Printf("Closed manual connection to rabbitmq")
