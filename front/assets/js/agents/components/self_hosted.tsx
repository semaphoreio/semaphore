...
export interface Instruction {
  name: string;
  icon?: string;
  iconElement?: h.JSX.Element;
  Component: h.JSX.Element;
}

export const InstallationInstructions = () => {
  const [selectedOs, setSelectedOs] = useState<types.Instruction>();

  const selectOs = (os: Instruction) => {
    setSelectedOs(os);
  };

  const instructions: Array<Instruction> = [
    {
      name: Instructions.AWS.name,
      icon: Instructions.AWS.icon,
      Component: <Instructions.AWS.Component/>,
    },
    {
      name: Instructions.Kubernetes.name,

      iconElement: (
        <div style={{ width: 64, height: 64 }} className="center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="64"
            height="64"
            viewBox="0 0 722.846 722.846"
          >
            <path
      ...
      Component: <Instructions.Kubernetes.Component/>,
    },
    {
      name: Instructions.MacOs.name,
      icon: Instructions.MacOs.icon,
      Component: <Instructions.MacOs.Component/>,
    },
    {
      name: Instructions.Ubuntu.name,
      icon: Instructions.Ubuntu.icon,
      Component: <Instructions.Ubuntu.Component/>,
    },
    {
      name: Instructions.Linux.name,
      icon: Instructions.Linux.icon,
      Component: <Instructions.Linux.Component/>,
    },
    {
      name: Instructions.RHEL.name,
      icon: Instructions.RHEL.icon,
      Component: <Instructions.RHEL.Component/>,
    },
    {
      name: Instructions.Windows.name,
      icon: Instructions.Windows.icon,
      Component: <Instructions.Windows.Component/>,
    },
  ];

  useEffect(() => {
    if (selectedOs) return;
    selectOs(instructions[0]);
  }, []);

  const currentOsName = selectedOs ? selectedOs.name : ``;

  return (
    <div className="shadow-1 bg-white pa3 mv3 br3">
      <p className="mb3">
        Follow the instructions and you should see the agent running here
        shortly.
      </p>
      <div className="flex ph3-l">
        {instructions.map((os: Instruction) => (
          <components.OsBox
            key={os.name}
            name={os.name}
            icon={os.icon}
            iconElement={os.iconElement}
            active={currentOsName === os.name}
            onClick={() => selectOs(os)}
          />
        ))}
      </div>
      {selectedOs && <div className="mt4">{selectedOs.Component}</div>}
    </div>
  );
};
