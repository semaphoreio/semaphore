...
export const TestExplorer = ({ className }: { className: string }) => {
  ...
  useEffect(() => {
    ...
    if (failedSuites.length > 0) {
      if (
        ...
        !failedSuites.find((suite) => navigation.state.activeSuiteId == suite.id)
      ) {
        return;
      }
  ...
  return (
    <div className={className} ref={el}>
      ...
      {filter.state.excludedStates.length != 0 && (
        <FilterInfo suites={activeReport.suites} filteredSuites={filteredSuites}/>
      )}
    </div>
  );
};
  ...
  return (
    <div data-search-box="">
      <div className="mt1 mb2 f6 tc gray">
        ...
        <span>
          <a
            className="gray hover-dark-gray underline pointer h7"
            onClick={() => filter.dispatch({ type: `SET_QUERY`, query: `` })}
          >
            Clear search
          </a>
        </span>
