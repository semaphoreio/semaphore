  ...
  module GrpcDsl
    ...
    def define_rpc(method_name, &block)
      ...
      define_method(method_name) do |request, call|
        ...
        InternalApi::GrpcDsl.with_metrics(namespace, method_name) do
          InternalApi::GrpcDsl.with_managed_db_connection do
            InternalApi::GrpcDsl.with_logs(namespace, method_name, call.metadata) do |logger|
              InternalApi::GrpcDsl.with_recorded_exceptions(request) do
                stop = Time.now.to_ms
                Watchman.submit("semaphore2_grpc_dsl_overhead", stop - start, :timing, tags: [namespace, method_name])

                instance_exec(request, logger, call, &block)
              end
            end
          end
        end
    ...
    def with_logs(service_name, method_name, metadata)
      Logman.process("#{service_name}-#{method_name}", :id => SecureRandom.uuid) do |logger|
        logger.info("Request headers #{metadata}")
        time = Time.zone.now

        response = yield(logger)
    ...
    def with_metrics(service_name, method_name)
      db_pool_stat = ActiveRecord::Base.connection_pool.stat
      hostname = Socket.gethostname

      db_pool_stat.each do |metric, value|
        Watchman.submit("activerecord.db_pool", value, :gauge, tags: [metric, service_name, hostname])
      end
