...
const DeploymentCardStage = React.memo(({ data, selected, onIconAction, id, onDelete }) => {
  ...
  return (
    <div className={`bg-white br2 ba bw1  ${selected ? 'b--indigo' : 'b--lighter-gray'} relative`} style={nodeStyle}>
      ...
      {/* Last Run info */}
      <div className={`${data.status === 'Passed' ? 'bg-washed-green b--green' : data.status === 'Failed' ? 'bg-washed-red b--red' : data.status === 'Running' ? 'bg-washed-blue b--indigo' : data.status === 'Queued' ? 'bg-washed-yellow b--yellow' : 'bg-washed-green b--green'} w-full bt min-w-0 text-ellipsis overflow-hidden`}>
      
        <div className="pa2">
  <div className="flex min-w-0 items-center">
    {/* Status Icon */}
    <div className="mr2 flex-shrink-0 flex items-center justify-center" style={{height: '32px'}}>
      {(() => {
        switch (data.status.toLowerCase()) {
          case 'passed':
            return <span className="material-icons" style={{ color: '#24b47e', fontSize: 24, verticalAlign: 'middle', lineHeight: 1 }} title="Passed">check_circle</span>;
          case 'failed':
            return <span className="material-icons" style={{ color: '#e74c3c', fontSize: 24, verticalAlign: 'middle', lineHeight: 1 }} title="Failed">cancel</span>;
          case 'queued':
            return <span className="material-icons" style={{ color: '#ff9800', fontSize: 24, verticalAlign: 'middle', lineHeight: 1 }} title="Queued">pending</span>;
          case 'running':
            return <span className="material-icons spin" style={{ color: '#3498db', fontSize: 24, verticalAlign: 'middle', lineHeight: 1 }} title="Running">pending</span>;
          default:
            return null;
        }
      })()}
    ...
    <div className="flex-1 min-w-0">
      <a href="#" className="min-w-0 fw6 font-normal underline-hover truncate w-full db">
        BUG-213 When clicking on the button for starting a run, the run does not start
      </a>
    </div>

  </div>
  {/* Metadata row: Favicon and Project Name */}
  <div className="flex items-center mt1 w-100 ph1" style={{ minWidth: 0 }}>
    <div className="flex items-center min-w-0 flex-1">
      <img
        alt="Semaphore Logo"
        src={semaphore}
        className="h1 w1 mr2 flex-shrink-0"
        style={{ display: 'inline-block' }}
      />
      <a
        href={data.projectUrl || '#'}
        className="f7 gray underline-hover truncate"
        style={{ maxWidth: 180 }}
        target="_blank"
        rel="noopener noreferrer"
      >
        {data.projectName || 'proj-awesome'}
      </a>
    </div>
    <div className="f7 black-60 text-xs ml2 flex-shrink-0 tr" style={{ marginLeft: 'auto' }}>
      {data.timestamp}
    </div>
  </div>
</div>
                ...
                <div className="flex flex-wrap gap-1 min-w-0 badge-labels-container ph3 pb2 pt2 bt border-t-1 border-gray-400" style={{minWidth:0}}>
                  <span className="badge-label"><span className="badge-key">code:</span> <span className="badge-value">1042a82</span></span>
                  <span className="badge-label badge-label--highlight"><span className="badge-key">image:</span> <span className="badge-value">v.4.1.3</span></span>
                  <span className="badge-label"><span className="badge-key">terraform:</span> <span className="badge-value">v.2.9.2</span></span>
                  <span className="badge-label"><span className="badge-key">type:</span> <span className="badge-value">community</span></span>
                </div>
...
const GitHubIntegration = ({ data, selected }) => {
  ...
  return (
    ...
    <span className="material-symbols-outlined text-indigo-600" style={{ fontSize: 24, verticalAlign: 'middle' }}>podcasts</span>
    </div>
    <p className="mb0 b ml1">{data.repoName}</p>
    </div>
    </div>
    ...
    <div className="flex w-full h-full" ref={reactFlowWrapper}>
      ...
      <button
        ...
        style={{
          ...
          fontWeight: 600,
          cursor: 'pointer',
          boxShadow: '0 2px 8px rgba(128,128,128,0.20)'
        }}
      >
        Export as Image
      </button>
...
// Badge label styles for inline tags
...
if (typeof document !== 'undefined' && !document.getElementById('badge-label-styles')) {
  const style = document.createElement('style');
  style.id = 'badge-label-styles';
  style.innerHTML = badgeLabelStyles;
