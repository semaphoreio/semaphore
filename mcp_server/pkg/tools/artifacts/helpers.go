...
var fullLogsCandidates = []string{
	"agent/job_logs.txt",
	"agent/job_logs.txt.gz",
}

type artifactCallMetrics interface {
	IncrementTotal()
	IncrementSuccess()
	IncrementFailure()
...
var newArtifactCallMetrics = func(ctx context.Context, operation, orgID string) artifactCallMetrics {
	return shared.NewToolMetrics(ctx, "artifacts_"+operation, orgID)
}

func listPath(ctx context.Context, api internalapi.Provider, orgID, artifactID, directory string) ([]*artifacthubpb.ListItem, error) {
	resp, err := listPathRPC(ctx, api, orgID, &artifacthubpb.ListPathRequest{
		ArtifactId: artifactID,
		Path:       directory,
		// Explicitly keep directory unwrapping disabled for all artifact path listings.
		UnwrapDirectories: false,
	})
	if err != nil {
		return nil, formatArtifacthubPathError("ListPath", err)
	}
...
func listPathRPC(
	...
	api internalapi.Provider,
	orgID string,
	req *artifacthubpb.ListPathRequest,
) (resp *artifacthubpb.ListPathResponse, err error) {
	metrics := newArtifactCallMetrics(ctx, "list_path_rpc", orgID)
	if metrics != nil {
...
func resolveSignedURLPath(
	...
	scopeID string,
	relativePath string,
) (requestPath string, resolvedPath string, err error) {
	requestPath = artifactPath(scope, scopeID, relativePath)
	if scope != scopeJobs || !isJobFullLogsRequest(relativePath) {
		return requestPath, relativePath, nil
	}

	resolvedPath, err = resolveJobFullLogsPath(ctx, api, orgID, artifactID, scopeID)
	if err != nil {
		return "", "", err
	}

	return artifactPath(scopeJobs, scopeID, resolvedPath), resolvedPath, nil
}

func isJobFullLogsRequest(relativePath string) bool {
	cleaned := strings.Trim(strings.TrimSpace(relativePath), "/")
	switch cleaned {
	case "job_logs.txt", "job_logs.txt.gz", "agent/job_logs.txt", "agent/job_logs.txt.gz":
		return true
	default:
		return false
	}
...
func resolveJobFullLogsPath(
	...
	items, err := listPath(ctx, api, orgID, artifactID, artifactPath(scopeJobs, jobID, "agent/"))
	if err != nil {
		return "", err
	}
	...
	for _, item := range items {
		...
		relative := strings.Trim(toRelativeArtifactPath(item.GetName(), scopeJobs, jobID), "/")
		if relative == "" {
			continue
		}
	...
	for _, candidate := range fullLogsCandidates {
...
func getSignedURL(
	...
	path string,
	method string,
) (item signedURLItem, err error) {
	metrics := newArtifactCallMetrics(ctx, "get_signed_url_rpc", orgID)
	if metrics != nil {
		metrics.IncrementTotal()
		start := time.Now()
		defer func() {
	...
	if err != nil {
		...
		return signedURLItem{}, formatArtifacthubPathError("GetSignedURL", err)
	}

	url := strings.TrimSpace(resp.GetUrl())
	if url == "" {
		return signedURLItem{}, fmt.Errorf("artifact service returned malformed response: no signed URL")
	}
...
func formatArtifacthubPathError(operation string, err error) error {
	...
	if ok {
		...
		case grpccodes.FailedPrecondition:
			...
			if message == "" {
				return fmt.Errorf("artifacthub %s failed precondition", operation)
			}
			return fmt.Errorf("artifacthub %s failed precondition: %s", operation, message)
		}
	}
