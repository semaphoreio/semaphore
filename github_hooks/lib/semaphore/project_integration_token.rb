module Semaphore
  class ProjectIntegrationToken
    def project_token(project)
      ...
      when "github_app"
        github_app_token(project.repo_owner_and_name, :repository_remote_id => project.repository.remote_id)
      when "bitbucket"
    ...
    def github_app_token(repository_slug = nil, repository_remote_id: nil)
      Semaphore::GithubApp::Token.repository_token(repository_slug, :repository_remote_id => repository_remote_id)
    end
  end
end
