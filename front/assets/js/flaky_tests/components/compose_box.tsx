...
const Panel = (
  ...
  return (
    ...
    >
      {state.status === `stalled` && !state.isOpen && <AutocompleteSpinner/>}
      {state.isOpen &&
        state.collections.map(({ source, items }) => {
          return (
            <div
              key={`source-${source.sourceId as string}`}
              className={[`autocomplete-source`, state.status === `stalled` && `autocomplete-source-stalled`].filter(Boolean).join(` `)}
            >
              {items.length > 0 && (
    ...
    return [
      {
        ...
        sourceId: `filters`,
        getItems() {
          return activeToken?.word
            ? allowedParams.filter((item) => item.label.toLowerCase().includes(activeToken.word.slice(1).toLowerCase()))
            : allowedParams;
        },
      },
    ];
