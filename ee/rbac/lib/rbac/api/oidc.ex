defmodule Rbac.Api.OIDC do
  ...
  def get_oidc_federeted_identities(user) do
    case Rbac.FrontRepo.RepoHostAccount.list_for_user(user.id) do
      {:ok, list} ->
        list
  ...
  def client do
    middleware = [
      ...
      {Tesla.Middleware.BearerAuth, token: Rbac.OIDC.get_api_token!()},
      Tesla.Middleware.JSON,
      Tesla.Middleware.Logger,
      {Rbac.Api.Middleware.UpdateToken, token_fetcher: &Rbac.OIDC.get_api_token/0},
  ...
  defp base_url do
    Application.get_env(:rbac, :oidc)[:manage_url]
  end
end
