    ...
    if raw_schedulers do
      ...
      |> Enum.map(fn scheduler ->
        Scheduler.new(
          id: scheduler["id"] || "",
          name: scheduler["name"],
          branch: scheduler["branch"] || construct_reference(scheduler["reference_type"], scheduler["reference_name"]),
          at: scheduler["at"],
    ...
    if raw_tasks do
      ...
      |> Enum.map(fn task ->
        SpecTask.new(
          ...
          branch: task["branch"] || construct_reference(task["reference_type"], task["reference_name"]) || "",
          at: task["at"] || "",
