defmodule Front.RBAC.PermissionsTest do
  use ExUnit.Case
  ...
  describe "has? with permission patrol" do
    setup do
      Application.put_env(:front, :use_rbac_api, false)
    end
    ...
    test "When permission patrol crashes, every permission is false" do
      ...
      with_mocks([{Watchman, [:passthrough], [increment: fn _ -> nil end]}]) do
        has_permissions = Front.RBAC.Permissions.has?(user_id, org_id, permissions)

        assert_called_exactly(Watchman.increment("permissions.has.failure"), 1)
    ...
    test "when user_id is nil and only one permissions is passed" do
      resp = Front.RBAC.Permissions.has?(nil, Ecto.UUID.generate(), "organization.view")
      assert resp == false
    end

    test "when user_id is nil and multiple permissions are passed" do
      permissions = ["organization.view", "organization.delete"]
      resp = Front.RBAC.Permissions.has?(nil, Ecto.UUID.generate(), permissions)
    ...
    test "when user_id is '' and only one permissions is passed" do
      resp = Front.RBAC.Permissions.has?("", Ecto.UUID.generate(), "organization.view")
      assert resp == false
    end
  end
  ...
  describe "has? with rbac API" do
    setup do
      Application.put_env(:front, :use_rbac_api, true)
    ...
    test "No permission is asked for -> all permissions in RBAC response are used" do
      ...
      user_id = Ecto.UUID.generate()
      org_id = Ecto.UUID.generate()
      has_permissions = Front.RBAC.Permissions.has?(user_id, org_id, [])
      assert has_permissions["organization.view"] == true
