defmodule PipelinesAPI.Roles.Authorize do
  ...
  def authorize_view_roles(conn, _opts) do
    is_authorized?("organization.custom_roles.view", conn)
  end

  def authorize_manage_roles(conn, _opts) do
    is_authorized?("organization.custom_roles.manage", conn)
  end
  ...
  defp is_authorized_check(permission, conn, user_id, org_id) do
    with params <- %{user_id: user_id, org_id: org_id},
      ...
      {:error, {:internal, _}} = error ->
        LT.error(error, "Roles.Authorize")
