...
export class Parameter {
  ...
  regexPatternValidationMsg() {
    ...
    if (byteLength(this.regex_pattern) > MAX_REGEX_PATTERN_LENGTH) {
      return `Pattern is too long (max ${MAX_REGEX_PATTERN_LENGTH} bytes)`
    }
    try {
  ...
  defaultValueValidationMsg() {
    ...
    if (byteLength(this.regex_pattern) > MAX_REGEX_PATTERN_LENGTH) { return }
    if (!this.default_value || this.default_value.length < 1) { return }
    if (byteLength(this.default_value) > MAX_PARAM_VALUE_LENGTH) {
      return `Default value is too long (max ${MAX_PARAM_VALUE_LENGTH} bytes)`
    }

    let regex
    try {
      regex = new RegExp(this.regex_pattern)
    } catch (_err) {
      return
    }
    if (!regex.test(this.default_value)) {
