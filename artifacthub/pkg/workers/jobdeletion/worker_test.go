...
func Test__JobDeletionWorker(t *testing.T) {
	...
	t.Run("it deletes artifacts when job deletion event is received", func(t *testing.T) {
		...
		err = bucket.CreateObject(ctx, jobPath+"file1.txt", []byte("content1"))
		assert.Nil(t, err)
		err = bucket.CreateObject(ctx, jobPath+"file2.txt", []byte("content2"))
		assert.Nil(t, err)
		...
		assert.Eventually(t, func() bool {
			exists, _ := bucket.IsFile(ctx, jobPath+"file1.txt")
			return !exists
		}, 10*time.Second, 100*time.Millisecond)

		exists, _ := bucket.IsFile(ctx, jobPath+"file2.txt")
