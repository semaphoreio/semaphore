defmodule Rbac.OIDC.FederatedIdentitySync do
  ...
  @failure_metric "rbac.federated_identity_sync.failure"
  @max_attempts 3
  ...
  defp push_github_identity(account) do
    case Rbac.Store.OIDCUser.fetch_by_user_id(account.user_id) do
      {:ok, oidc_user} ->
        ...
        with_retries(fn ->
          Rbac.OIDC.User.set_federated_identity(oidc_user.oidc_user_id, identity)
        end)
        |> case do
          {:ok, _} ->
            Logger.info(
  ...
  defp backoff_ms do
    Application.get_env(:rbac, :federated_identity_sync_backoff_ms, 500)
  end
