...
// configureMetrics blocks while it retries the statsd connection (up to
// 30 x 2s), so it must run in a goroutine — metric calls before it finishes
// are safe no-ops, and the HTTP server must not wait on a metrics backend.
