defmodule Guard.Id.Api do
  ...
  defp assign_user_info(conn, _opts) do
    # Normalize the x-semaphore-user-id header to either a single non-empty id or
    # nil: a missing header, a blank value, or anything other than exactly one
    # value all mean "no user". This keeps assigns[:user_id] presence checks
    # (e.g. the /signup logged_in flag) correct when the header is absent or blank.
    user_id =
      case conn |> get_req_header("x-semaphore-user-id") do
