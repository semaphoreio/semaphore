...
RSpec.describe RepoHost::Github::Payload do
  describe "PR approval commands" do
    it "recognizes /sem-approve as PR approval command" do
      ...
      expect(payload.pr_approval_include_cache?).to be(false)
    end
    ...
    it "recognizes /sem-approve --include-cache option" do
      payload = described_class.new(
        {
          "issue" => { "number" => 45, "pull_request" => { "url" => "https://example.test/pr/45" } },
          "comment" => { "body" => "/sem-approve --include-cache", "user" => { "login" => "octocat" } }
        }.to_json
      )

      expect(payload.pr_approval?).to be(true)
      expect(payload.pr_approval_include_secrets?).to be(false)
      expect(payload.pr_approval_include_cache?).to be(true)
    end
    ...
    it "recognizes /sem-approve with both include options" do
      payload = described_class.new(
        {
          "issue" => { "number" => 45, "pull_request" => { "url" => "https://example.test/pr/45" } },
          "comment" => { "body" => "/sem-approve --include-secrets --include-cache", "user" => { "login" => "octocat" } }
        }.to_json
      )
    ...
    it "recognizes /sem-approve with both include options in reverse order" do
      payload = described_class.new(
        {
          "issue" => { "number" => 45, "pull_request" => { "url" => "https://example.test/pr/45" } },
          "comment" => { "body" => "/sem-approve --include-cache --include-secrets", "user" => { "login" => "octocat" } }
        }.to_json
      )
    ...
    it "recognizes persisted include option markers on pull request payload" do
      ...
      pr_payload["semaphore_approval_include_cache"] = true
