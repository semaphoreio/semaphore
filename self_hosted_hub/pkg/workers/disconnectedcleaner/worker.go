...
// Start begins the disconnected agent cleaner process with the given publisher
...
func deleteDisconnectedAgents(db *gorm.DB, publisher *amqp.Publisher) error {
	...
	if err != nil {
		log.Errorf("Error querying disconnected agents for cleanup: %v", err)
		return err
	}
	...
	log.Infof("Found %d disconnected agents to clean up", len(agents))
	...
	for _, agent := range agents {
		...
		if agent.AssignedJobID != nil && *agent.AssignedJobID != "" {
			...
			log.Infof("Disconnected agent %s with job %s is being cleaned, marking job as failed", 
				agent.ID, jobID)
			
			err := publisher.HandleJobFinished(ctx, jobID, "failed")
			if err != nil {
				log.Errorf("Failed to publish job finalization for job %s: %s", 
					jobID, err.Error())
	...
	if len(idsToDelete) == 0 {
		log.Infof("No agents to delete after processing")
		return nil
	}
	
	// Delete only the agents we've processed successfully
	log.Infof("Deleting %d disconnected agents", len(idsToDelete))
	dbExec := db.Exec(`DELETE FROM agents WHERE id IN (?)`, idsToDelete)
	if dbExec.Error != nil {
		log.Errorf("Error deleting disconnected agents: %v", dbExec.Error)
		return dbExec.Error
	}

	if dbExec.RowsAffected != int64(len(idsToDelete)) {
		log.Errorf("Fewer agents were deleted than expected: expected %d, got %d", 
			len(idsToDelete), dbExec.RowsAffected)
	}
