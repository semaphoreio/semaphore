defmodule InternalClients.Projecthub.GrpcClient do
  @moduledoc """
  Module is used for making gRPC calls to Projecthub service.
  """

  alias InternalApi.Projecthub, as: API
  alias PublicAPI.Util.{Log, Metrics}
  alias API.ProjectService.Stub
  require Logger
  ...
  def send_request({:ok, request}) do
    ...
    case maybe_result do
      {:ok, result} -> result
      {:error, reason} -> Log.internal_error(reason, "")
    end
  end
  ...
  defp req_to_func(%API.DescribeManyRequest{}), do: &Stub.describe_many/3
