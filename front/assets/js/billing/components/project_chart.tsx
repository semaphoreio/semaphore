...
export const ProjectChart = (props: Props) => {
  ...
  useLayoutEffect(() => {
    ...
    const colorScale = () => d3.scaleOrdinal<string, string>().domain(project.metricNames()).range(d3.schemeTableau10);
  ...
  const aggregatePlotData = (
    plotData: components.Charts.PlotData[],
    aggregationType: AggregationType
  ): components.Charts.PlotData[] => {
    let value = 0;
    ...
    >
      <div className="f6">
        ...
        {_.map(metric.details, (detailValue, detailName) => {
          return (
            <div className={`flex justify-between ${selectedDetailName == detailName ? `b` : ``}`}>
              <div>{detailName}</div>
        ...
        <div className="gray f6 pointer">
          <div className="tr inline-flex items-center">
            {labels.map((label, idx) => {
              ...
              return (
                <div key={idx} className={`inline-flex items-center ${classes}`} onClick={() => selectMetric(label)}>
                  <span
                    className="mr2 ml3 dib"
                    style={`width: 10px; height: 10px; background: ${props.colorScale(label)};`}
                  ></span>
