  ...
  defp handle_fetch_token(repo_host_account) do
    ...
    case Tesla.post(client, @oauth_path, body_params) do
      ...
      {:ok, %Tesla.Env{status: status, body: body}} when status in 400..499 ->
        Logger.warning("Failed to refresh gitlab token for #{repo_host_account.login}, with: #{inspect(body)}")
        {:error, :revoked}

      {:ok, %Tesla.Env{status: _status, body: body}} ->
        Logger.debug("Failed to refresh gitlab token for #{repo_host_account.login}, with: #{inspect(body)}")
        {:error, :failed}
