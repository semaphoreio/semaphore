...
func TestArtifactsListArtifactsAPIFeatureDisabled(t *testing.T) {
	...
	if !strings.Contains(strings.ToLower(msg), "artifact operations are disabled") {
		t.Fatalf("expected artifacts_api disabled error, got %q", msg)
	}
}

func TestArtifactsSignedURLArtifactsAPIFeatureDisabled(t *testing.T) {
	provider := &support.MockProvider{
		Timeout: time.Second,
		FeaturesService: support.FeatureClientStub{
			States: map[string]feature.State{
