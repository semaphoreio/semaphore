defmodule Secrethub.DeploymentTargets.MapperTest do
  ...
  describe "encode/1" do
    test "encodes basic fields as metadata", ctx do
      ...
      assert metadata.level == :DEPLOYMENT_TARGET
    end

    test "encodes deployment target ID in a config", ctx = %{dt_id: dt_id} do
      assert %API.Secret{dt_config: %API.Secret.DTConfig{deployment_target_id: ^dt_id}} =
  ...
  describe "decode/1" do
    test "encodes basic fields as metadata", ctx do
      expected = %{
        ...
        dt_id: ctx.params.dt_config.deployment_target_id,
    ...
    test "when field is empty string then omits it", ctx do
      ...
      refute %{ctx.params | dt_config: nil}
             |> Mapper.decode()
             |> Map.has_key?(:dt_id)

      refute %{ctx.params | dt_config: API.Secret.DTConfig.new()}
             |> Mapper.decode()
             |> Map.has_key?(:dt_id)
    end
